...
void Graph::distAtK(int k, int root)
{
    int dist[nVertices + 1];
    int stock[nVertices + 1];
    ...
    for(int i = 0; i < k; i++)
    {
        ...
        {
            for(int a = 0; a < adjList[j].getLength(); a++)
            {
                edge edge1 = adjList[j].getItem(a);

                if(stock[edge1.ver] > edge1.weight + dist[j])
                {
                    stock[edge1.ver] = edge1.weight + dist[j];
                }
            }
        }
