    ...
    for(int i = 1; i <= nVertices; i++)
    {
        for(int j = 1; j <= nVertices; j++)
        {
            if(graph[i][j] != INFINITY)
            {
                ...
                if(graph[j][i] == INFINITY and flow[i][j]!=0) residual[j][i]=flow[i][j];
...
int maxFlow(int source, int sink, int *parent, int **graph)
{
   ...
   while(1){
    ...
    if(graph[parent[sink]][sink]< f) f=graph[parent[sink]][sink];
   }
...
void makeFlow(int source, int sink, int *parent, int **flow, int **graph,int f)
{
    if(isExistPath(source, sink, parent))
    {
        while(sink != source)
        {
            if(graph[parent[sink]][sink]!=INFINITY) flow[parent[sink]][sink]+=f;
            else flow[parent[sink]][sink]-=f;
            sink = parent[sink];
        }
    }
}
...
void FordFulkerson(int source, int sink, int nVertices, int **graph, int **flow, int **residual)
{
  ...
  //  isExistGraph(source, sink, parent);
    int count=0;
    int x = 0;
